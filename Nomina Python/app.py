...
def realizar_pago():
    ...
    cursor.execute("SELECT Nombres, Apellido_Paterno, Apellido_Materno, Salario_Base, FROM empleados WHERE ID_empleado = %s", (empleado_id,))
    empleado_data = cursor.fetchone()

    cursor.execute("SELECT Riesgo_LV FROM Cargos WHERE ID_Cargo = %s", (empleado_id,))
    cargo_data = cursor.fetchone()

    empleado_nombre = Nombres + Apellido_Paterno + Apellido_Materno
